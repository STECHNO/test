...
export const loadDepartments = () => dispatch => {
    let arrayOfTypes = [];
    // axios.get('http://localhost:5001/api/dpt')
    axios.get('http://localhost:5001/api/dpt')
        .then(({ data }) => {
...
//     let arrayOfTypes = [];
//     axios.get('http://localhost:5007/api/product')
//         .then(({ data }) => {
...
export const loadCourses = () => dispatch => {
    let arrayOfTypes = [];
    axios.get('http://localhost:5001/api/crs')
        .then(({ data }) => {
...
export const loadTerms = () => dispatch => {
    let arrayOfTypes = [];
    axios.get('http://localhost:5001/api/trm')
        .then(({ data }) => {
...
export const loadSections = () => dispatch => {
    let arrayOfTypes = [];
    axios.get('http://localhost:5001/api/sec')
        .then(({ data }) => {
            data.data.filter(({ _id, name }) => {
                const terms = { _id, name }
                arrayOfTypes.push(terms);
